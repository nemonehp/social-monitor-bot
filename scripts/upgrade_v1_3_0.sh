...
if [[ "${CONFIRM_V1_3_0_UPGRADE:-}" != "YES" ]]; then
  echo "Set CONFIRM_V1_3_0_UPGRADE=YES to continue." >&2
  exit 2
fi
...
db_backup="$BACKUP_DIR/pre-v1.3.0-$stamp.dump"
env_target="$(readlink -f .env)"
env_backup="$BACKUP_DIR/pre-v1.3.0-$stamp.env"
...
if missing:
    if output and output[-1].strip():
        output.append("")
    output.append("# v1.3.0 capacity, affinity and integrity protection")
...
printf 'Reconnect the signal chat from the admin menu to create forum topics.\n'
