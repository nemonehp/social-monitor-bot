...
if [[ "${CONFIRM_V1_3_1_UPGRADE:-}" != "YES" ]]; then
  echo "Set CONFIRM_V1_3_1_UPGRADE=YES to continue." >&2
  exit 2
fi
...
db_backup="$BACKUP_DIR/pre-v1.3.1-$stamp.dump"
env_target="$(readlink -f .env)"
env_backup="$BACKUP_DIR/pre-v1.3.1-$stamp.env"

printf '[1/8] Environment backup and capacity settings: %s\n' "$env_backup"
...
if missing:
    if output and output[-1].strip():
        output.append("")
    output.append("# v1.3.1 independent platform cadence hotfix")
...
printf 'Forum topic service messages are now ignored silently; reconnecting the chat is optional.\n'
