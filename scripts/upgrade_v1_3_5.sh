...
EXPECTED_REVISION="0007_integrity_observation"

if [[ "${CONFIRM_V1_3_5_UPGRADE:-}" != "YES" ]]; then
  echo "Set CONFIRM_V1_3_5_UPGRADE=YES to continue." >&2
  exit 2
fi
...
db_backup="$BACKUP_DIR/pre-v1.3.5-$stamp.dump"
env_target="$(readlink -f .env)"
env_backup="$BACKUP_DIR/pre-v1.3.5-$stamp.env"
...
printf 'Observed platform IDs now suppress false integrity gaps; delivery layout is deterministic.\n'
