...
EXPECTED_REVISION="0005_vk_account_identity"

if [[ "${CONFIRM_V1_3_3_UPGRADE:-}" != "YES" ]]; then
  echo "Set CONFIRM_V1_3_3_UPGRADE=YES to continue." >&2
  exit 2
fi
...
db_backup="$BACKUP_DIR/pre-v1.3.3-$stamp.dump"
env_target="$(readlink -f .env)"
env_backup="$BACKUP_DIR/pre-v1.3.3-$stamp.env"
...
if missing:
    if output and output[-1].strip():
        output.append("")
    output.append("# v1.3.3 VK account identity")
...
printf 'VK credentials are now identified by the verified account ID returned by users.get.\n'
